<template>
    <DocumentsRoot :modelValue="props.modelValue" @update:modelValue="value => emit('update:modelValue', value)"
        title=" " :receivable-id="props.receivableId">
        ...
        <div class="bg-white p-2 rounded-xl shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700 ">
            ...
            <div class="w-full overflow-x-auto border rounded-lg bg-indigo-50 rounded-xl p-3 border mb-5">
                ...
                <div :class="{
                    ...
                    'border-green-300 dark:border-green-700': receivableData.status !== 'pending',
                    'border-yellow-400 dark:border-yellow-600': receivableData.status === 'pending'
                    ...
                    <div
                        ...
                        <div class="flex flex-col w-full sm:w-auto text-center sm:text-left mb-2 sm:mb-0">
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                                Detalle del Pago N° {{ receivableData.sale.invoice_number }}
                            </h3>
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                Registrado el:
                                {{ new Date(receivableData.created_at).toLocaleDateString('es-CO', {
                                    hour: '2-digit', minute:
                                        '2-digit'
                                }) }}
                            </p>
                        ...
                        <div class="text-center sm:text-right w-full sm:w-auto">
                            <p class="text-2xl font-extrabold text-green-600 dark:text-green-400">
                               {{ formatCurrency(receivableData.amount) }}
                            </p>

                            <span :class="receivableData.status === 'pending'
                                ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300'
                                : 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300'"
                                class="px-3 py-1 text-xs font-semibold rounded-full mt-1 inline-block">
                                {{ receivableData.status === 'pending' ? 'PENDIENTE' : 'PAGADO' }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <span>{{ receivableData.payment_provider.name }}</span>
                        </div>

                        <div>
                            <span class="font-medium text-gray-500 dark:text-gray-400 block">N° Transacción:</span>
                            <span>{{ receivableData.sale.number_transactions || 'N/A' }}</span>
                        </div>

                        <div>
                            <span class="font-medium text-gray-500 dark:text-gray-400 block">Descuento Total:</span>
                            <span>{{ formatCurrency(receivableData.sale.discount_amount) }}</span>
                        </div>

                        <div class="col-span-1 sm:col-span-2">
                            <span class="font-medium text-gray-500 dark:text-gray-400 block">Cliente:</span>
                            <span>
                                {{ receivableData.client.name }}
                                (<span class="text-gray-500 dark:text-gray-400">{{ receivableData.client.phone
                                }}</span>)
                            </span>
                        </div>
                    </div>
                ...
                <hr class="mt-3 mb-3" />
                <form v-if="statusPayment == 'pendiente'" @submit.prevent="handleSubmit" class="flex flex-wrap items-start gap-6">

                    <input type="hidden" v-model="form.id">
                    <div class="flex flex-col w-full md:w-64">
                        <label class="text-sm font-medium text-gray-700">Monto Recibido por :  (<b>{{
                                receivableData.payment_provider.name
                                }}</b>)</label>
                        <input type="text" v-model="form.paid_amount" @input="form.paid_amount = $getFormatDecimal($event.target.value)" required
        ...
        w-full md:w-auto">
                ...
                <div v-if="statusPayment == 'pendiente'">
        ...
        w-full md:w-auto text-center gradient-button">
                    ...
                    <span class="leading-snug whitespace-nowrap text-center">
                        {{ loading ? 'Pagando Cuenta...' : 'Pagar Cuenta' }}
                    </span>
                </button>
                </div>
            </div>
...
const emit = defineEmits(['update:modelValue', 'documentSaved', "receivable-registered", "dataUpdated"])
...
// ----------------------------------------------------------------------
watch(() => props.receivableData, (newreceivableData) => {


    // Si newreceivableData es null o undefined, limpia el formulario
    if (!newreceivableData) {
        form.id = null;
    ...
    form.paid_amount = newreceivableData.amount;

    // console.log('Formulario precargado para el receivablee ID:', form.id);
}, { immediate: true });
...
const handleSubmit = async () => {
    ...
    // 🎯 El bucle ahora solo procesará 'id' y 'paid_amount'
    ...
    try {
        const response = await api.post(`/admin/accountsReceivable/${form.id}/pay`, formData);

        // La variable 'isUpdate' no existe, simplificamos el mensaje
        successMessage.value = response.data.message ?? 'Cuenta Pagada Éxitosamente.';

        Swal.fire({
            position: "top-end",
            icon: "success",
            title: 'Cuenta Pagada Éxitosamente.',
        ...
        emit("dataUpdated", {
            success: true,
            receivableId: form.id,
            data: response.data.receivable
        });
        emit("receivable-registered", {
            success: true,
